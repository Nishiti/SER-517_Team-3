    ...
    for field in header:
        ...
        if field == "website":
            if each[field] != "":
                ...
                web_soc_handles = web_soc_handles + each[field]
            else:
        ...
        row["website_social_media_handles"] = web_soc_handles

        if field == "Areas of interest":
            row["areas_of_interest"] = [x.strip()
                                        for x in each[field].split(',')
                                        if x != ""]
        ...
        # The gender field is being hardcoded as it is not available
        # in current sponsors data
        # and the gender field is a required field for the database
        row["gender"] = "Male"

        # The path of a default image is being hardcoded
        # as the available data from Sponsor
        ...
        # Add the row to the project database collection
        # if the row has valid data
    if bool(row):
